...
- 涉及网站与 Agent 集成、RCON 流程、命令 payload、审计展示或共享配置预期时，直接检查 `kepcs` 仓库，不要再次询问路径
...
- 只改当前目标需要的代码和文档，不做投机式重构
- 只有用户明确要求时才 `git commit`；提交标题用 Conventional Commits，只写标题
...
- 如果任务涉及跨仓库契约，要明确说明是否同时核对了 `kepcs`
- 最后回复要写清：改了哪些模块、跑了哪些验证、是否涉及 `config.yaml`、是否跨仓库核对、是否 commit / push
