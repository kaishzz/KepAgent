...
- When an example file adds fields or variables, add the missing fields to the real file.
- When an example file removes obsolete fields, remove the corresponding fields from the real file only if they are part of that same mirrored structure.
- When an example file renames or restructures fields, apply the same field-name change to the real file while preserving the user's existing values whenever possible.
- Sync structure, field names, and expected keys, but do not overwrite existing secrets, passwords, tokens, paths, or other user data unless explicitly asked.
